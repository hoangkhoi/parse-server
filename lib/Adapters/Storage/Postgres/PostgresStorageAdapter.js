...
var transformAggregateField = function transformAggregateField(fieldName) {
...
var PostgresStorageAdapter = exports.PostgresStorageAdapter = function () {
  ...
  _createClass(PostgresStorageAdapter, [{
    ...
    value: function distinct(className, schema, query, fieldName) {
      ...
      var isArrayField = schema.fields && schema.fields[fieldName] && schema.fields[fieldName].type === 'Array';
      var values = [field, column, className];
      var where = buildWhereClause({ schema: schema, query: query, index: 4 });
      values.push.apply(values, _toConsumableArray(where.values));

      var wherePattern = where.pattern.length > 0 ? 'WHERE ' + where.pattern : '';
      var qs = 'SELECT DISTINCT ON ($1:raw) $2:raw FROM $3:name ' + wherePattern;
      if (isArrayField) {
        qs = 'SELECT distinct jsonb_array_elements($1:raw) as $2:raw FROM $3:name ' + wherePattern;
      }
      debug(qs, values);
      return this._client.any(qs, values).catch(function () {
        return [];
      }).then(function (results) {
        if (fieldName.indexOf('.') === -1) {
          return results.map(function (object) {
        ...
        return results.map(function (object) {
          return object[column][child];
        });
      });
    }
  }, {
    key: 'aggregate',
    value: function aggregate(className, pipeline) {
      ...
      for (var i = 0; i < pipeline.length; i += 1) {
        ...
        if (stage.$group) {
          for (var field in stage.$group) {
            var value = stage.$group[field];
        ...
        if (stage.$project) {
          ...
          for (var _field in stage.$project) {
            var _value2 = stage.$project[_field];
            if (_value2 === 1 || _value2 === true) {
              columns.push(_field);
            }
          }
        }
        if (stage.$match) {
          (function () {
            ...
            var _loop3 = function _loop3(_field2) {
              var value = stage.$match[_field2];
              Object.keys(ParseToPosgresComparator).forEach(function (cmp) {
                if (value[cmp]) {
                  var pgComparator = ParseToPosgresComparator[cmp];
                  patterns.push(_field2 + ' ' + pgComparator + ' ' + value[cmp]);
                }
              });
            };

            for (var _field2 in stage.$match) {
              _loop3(_field2);
            }
      ...
      var qs = 'SELECT ' + columns + ' FROM $1:name ' + wherePattern + ' ' + sortPattern + ' ' + limitPattern + ' ' + skipPattern + ' ' + groupPattern;
      debug(qs, values);
